...
export function PageHero({ eyebrow, title, description, crumbs }: PageHeroProps) {
  return (
    <section className="relative overflow-hidden border-b border-white/10 bg-ink">
      ...
      <div className="pointer-events-none absolute -right-32 -top-32 h-96 w-96 rounded-full bg-racing/15 blur-[130px]" />

      <div className="relative mx-auto max-w-content px-5 py-16 sm:px-6 lg:px-8 lg:py-20">
        <nav aria-label="Fil d'ariane">
          <ol className="flex flex-wrap items-center gap-1.5 font-mono text-xs uppercase tracking-wider text-mute">
            {crumbs.map((crumb, i) => (
        ...
        <p className="mt-6 inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.22em] text-racing">
          <span className="h-px w-6 bg-racing" />
          {eyebrow}
        </p>
        <h1 className="mt-4 max-w-3xl font-tight text-4xl font-extrabold leading-tight tracking-tight text-white sm:text-5xl">
          {title}
        </h1>
        {description && (
